...
async def test_custom_formatting(mock_bot):
    ...
    await handler.emit(logger.makeRecord("test_custom", logging.INFO, "", 0, "Test message", (), None))
...
async def test_batch_processing(handler, mock_bot):
    ...
    for msg in ["Message 1", "Message 2", "Message 3"]:
        await handler.emit(logger.makeRecord("test_batch", logging.INFO, "", 0, msg, (), None))
...
async def test_retry_mechanism(mock_bot):
    ...
    record = logger.makeRecord("test_retry", logging.INFO, "", 0, "Test retry message", (), None)
...
async def test_multiple_chat_ids(mock_bot):
    ...
    # Send message directly
    await handler.emit(logger.makeRecord("test_multi", logging.INFO, "", 0, "Multi-chat message", (), None))
    await asyncio.sleep(0.1)
...
async def test_queue_error_handling(mock_bot):
    ...
    # Send message directly
    record = logger.makeRecord("test_queue_error", logging.INFO, "", 0, "Test error message", (), None)
...
async def test_shutdown_handling(mock_bot):
    ...
    await handler.emit(logger.makeRecord("test_shutdown", logging.INFO, "", 0, "Pre-shutdown message", (), None))
    await asyncio.sleep(0.1)
    ...
    await handler.emit(logger.makeRecord("test_shutdown", logging.INFO, "", 0, "Post-shutdown message", (), None))
    await asyncio.sleep(0.1)
...
async def test_async_context_manager(mock_bot):
    ...
    async with CustomTelegramHandler(
        ...
        # Send message directly
        await handler.emit(logger.makeRecord("test_context", logging.INFO, "", 0, "Context message", (), None))
        await asyncio.sleep(0.1)
        assert mock_bot.send_message.call_count == 1
...
async def test_custom_level_emojis(mock_bot):
    ...
    # Send message directly
    await handler.emit(logger.makeRecord("test_emoji", logging.INFO, "", 0, "Emoji test", (), None))
    await asyncio.sleep(0.1)
...
async def test_signal_handling(mock_bot):
    ...
    await handler.emit(logger.makeRecord("test_signal", logging.INFO, "", 0, "Post-signal message", (), None))
    await asyncio.sleep(0.1)
...
async def test_custom_date_format(mock_bot):
    ...
    # Send message directly
    await handler.emit(logger.makeRecord("test_date", logging.INFO, "", 0, "Date format test", (), None))
    await asyncio.sleep(0.1)
...
async def test_project_name_and_hashtags(mock_bot):
    ...
    # Send message directly
    await handler.emit(logger.makeRecord("test_project", logging.INFO, "", 0, "Project test", (), None))
    await asyncio.sleep(0.1)
...
async def test_html_formatting(mock_bot):
    ...
    # Send message directly
    await handler.emit(logger.makeRecord("test_html", logging.INFO, "", 0, "<b>Bold text</b> and <i>italic</i>", (), None))
    await asyncio.sleep(0.1)
...
async def test_queue_overflow_handling(handler, mock_bot):
    ...
    for i in range(10):
        await handler.emit(logger.makeRecord("test_overflow", logging.INFO, "", 0, f"Message {i}", (), None))
...
async def test_message_formatting_error_handling(mock_bot):
    ...
    # Send message directly
    await handler.emit(logger.makeRecord("test_format_error", logging.INFO, "", 0, "Test message", (), None))
    await asyncio.sleep(0.1)
...
async def test_signal_handling_with_cleanup(mock_bot):
    ...
    # Try sending after signal
    record = logging.LogRecord("test_signal", logging.INFO, "", 0, "Post-signal message", (), None)
...
async def test_queue_processing_error_handling(mock_bot):
    ...
    for i in range(4):
        try:
            await handler._send_message(TEST_CHAT_ID, f"Message {i}")  # Send directly to avoid batching
...
async def test_batch_processing_with_force(mock_bot):
    ...
    for i in range(3):  # Less than batch size
        await handler.emit(logger.makeRecord("test_batch_force", logging.INFO, "", 0, f"Message {i}", (), None))
...
async def test_graceful_shutdown_with_pending_messages(mock_bot):
    ...
    # Send messages
    for i in range(3):
        await handler.emit(logger.makeRecord("test_shutdown", logging.INFO, "", 0, f"Message {i}", (), None))
...
async def test_custom_formatter_with_all_features(mock_bot):
    ...
    await handler.emit(logger.makeRecord("test_format", logging.INFO, "", 0, "Info message", (), None))
    await handler.emit(logger.makeRecord("test_format", logging.ERROR, "", 0, "Error message", (), None))
...
async def test_event_loop_initialization():
    ...
    try:
        with patch('telegram.Bot') as mock_bot_class, \
             patch('asyncio.run', new=AsyncMock()) as mock_run:
            mock_bot = MagicMock()
            mock_bot.get_me = AsyncMock(return_value={"id": 123456789, "is_bot": True, "first_name": "Test Bot"})
            ...
            # Send a test message
            record = logging.LogRecord("test_init", logging.INFO, "", 0, "Test message", (), None)
            await handler.emit(record)
            await asyncio.sleep(TEST_SLEEP * 2)
...
async def test_signal_handler_setup():
    ...
    try:
        with patch('telegram.Bot') as mock_bot_class, \
            ...
            assert not handler._shutdown_complete.is_set()  # Not complete until close() is called
    finally:
        if handler:
            await handler.close()
...
async def test_batch_sender_thread():
    ...
    try:
        with patch('telegram.Bot') as mock_bot_class, \
            ...
            assert handler.message_queue[TEST_CHAT_ID].qsize() <= 1  # Last message might still be in queue
    finally:
        if handler:
            await handler.close()
...
async def test_graceful_shutdown_cleanup(mock_bot):
    ...
    await handler.emit(logging.LogRecord("test", logging.INFO, "", 0, "Message 4", (), None))
...
async def test_message_formatting_with_errors(mock_bot):
    ...
    await handler.emit(logger.makeRecord("test_format_error", logging.INFO, "", 0, "error", (), None))
    await asyncio.sleep(TEST_SLEEP)
    
    # Send normal message
    await handler.emit(logger.makeRecord("test_format_error", logging.INFO, "", 0, "normal", (), None))
    await asyncio.sleep(TEST_SLEEP)
